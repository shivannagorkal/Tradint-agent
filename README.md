# Confluence — Hybrid Multi-Agent Quantitative Trading Platform
...
Confluence unifies multi-model generative AI debate workflows with quantitative statistical rigor, algorithmic risk checks, and real-time order routing.
...
The API Gateway is the central nervous system of Confluence, built with **Node.js**, **Express**, and **TypeScript**.
...
Confluence avoids generic single-model bottlenecks. Instead, it routes each sub-task to the specific LLM architecture optimized for that domain.
...
Open **http://localhost:5173** to access the Confluence platform.
